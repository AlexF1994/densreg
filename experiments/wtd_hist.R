wtd.hist <- function (x, breaks = "Sturges", freq = NULL, probability = !freq, 
          ...
          col = NULL, border = NULL, main = paste("Histogram of", 
  ...
  if (use.br) 
    ...
    if (is.character(breaks)) {
      ...
      breaks <- switch(breaks, sturges = nclass.Sturges(x), 
                       `freedman-diaconis` = , fd = nclass.FD(x), 
                       scott = nclass.scott(x), stop("unknown 'breaks' algorithm"))
    }
  ...
  if (plot) {
    ...
    nf <- names(formals())
    nf <- nf[is.na(match(nf, c("x", "breaks", 
                               "nclass", "plot", "include.lowest", 
                               "weight", "right")))]
